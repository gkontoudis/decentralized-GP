function [mu_dist_comp,s2_dist_comp,mu_dist_1_hop,...
    ...
    dec_grbcm(mu_all,mu_comm,s2_all,s2_comm,s2_all_inv,s2_comm_inv,models,...
...
for j=1:nt
    ...
    if   nearest_neighbors(j)  > 1
        ...
        for i=1:n
            if i == 1
                beta_dist_comp(i,1) = 1;
            else
                beta_dist_comp(i,1) = .5*( log(s2_comm(j)) - log(s2_all_real(i)) );
            end
            z_dist_comp(i,1) =  beta_dist_comp(i,1)*s2_all_inv_real(i)*mu_all_real(i);
            l_dist_comp(i,1) =  beta_dist_comp(i,1)*s2_all_inv_real(i);
             
        end
            beta_dist_1(:,1) =  beta_dist_comp(:,1);
            beta_dist_2(:,1) =  beta_dist_comp(:,1);
        ...
        for i=1:iter_dac
            beta_dist_comp(:,i+1) = P_comp*beta_dist_comp(:,i);
            beta_dist_max(i+1) = max(beta_dist_comp(:,i+1));
            beta_dist_min(i+1) = min(beta_dist_comp(:,i+1));
            beta_dist_max_min(i+1) = beta_dist_max(i+1) - beta_dist_min(i+1);
            ...
            if z_dist_max_min(i+1) < thres_dac_max_min && l_dist_max_min(i+1) < thres_dac_max_min...
                    && beta_dist_max_min(i+1) < thres_dac_max_min
        ...
        for i=1:iter_dac
            beta_dist_1(:,i+1) = P_comp*beta_dist_1(:,i);
            beta_dist_max_1(i+1) = max(beta_dist_1(:,i+1));
            beta_dist_min_1(i+1) = min(beta_dist_1(:,i+1));
            beta_dist_max_min_1(i+1) = beta_dist_max_1(i+1) - beta_dist_min_1(i+1);
            ...
            if z_dist_max_min_1(i+1) < thres_dac_max_min && l_dist_max_min_1(i+1) < thres_dac_max_min...
                    && beta_dist_max_min_1(i+1) < thres_dac_max_min
        ...
        for i=1:iter_dac
            beta_dist_2(:,i+1) = P_comp*beta_dist_2(:,i);
            beta_dist_max_2(i+1) = max(beta_dist_2(:,i+1));
            beta_dist_min_2(i+1) = min(beta_dist_2(:,i+1));
            beta_dist_max_min_2(i+1) = beta_dist_max_2(i+1) - beta_dist_min_2(i+1);
            ...
            if z_dist_max_min_2(i+1) < thres_dac_max_min && l_dist_max_min_2(i+1) < thres_dac_max_min...
                    && beta_dist_max_min_2(i+1) < thres_dac_max_min
        ...
        if iter_dac_convrg_max_min(1,j) > n_all && n_all <= 5 % flooding
            ...
            s2_inv_dist_comp(j) = sum(l_dist_comp(:,1)) + (1-sum(beta_dist_comp(:,1)))*s2_comm_inv(j);
            s2_dist_comp(j) = (1/s2_inv_dist_comp(j));
            mu_dist_comp(j) = s2_dist_comp(j)*(sum(z_dist_comp(:,1))...
                +(1-sum(beta_dist_comp(:,1)))*s2_comm_inv(j)*mu_comm(j) )...
                *models{1}.Y_std + models{1}.Y_mean;
            ...
            s2_inv_dist_comp(j) = n*l_dist_comp(1,iter_dac_convrg_max_min(1,j))...
                + (1-n*beta_dist_comp(1,iter_dac_convrg_max_min(1,j)))*s2_comm_inv(j);
            s2_dist_comp(j) = (1/s2_inv_dist_comp(j));
            mu_dist_comp(j) = s2_dist_comp(j)*(n*z_dist_comp(1,iter_dac_convrg_max_min(1,j))...
                +(1-n*beta_dist_comp(1,iter_dac_convrg_max_min(1,j)))*s2_comm_inv(j)*mu_comm(j) )...
                *models{1}.Y_std + models{1}.Y_mean;
        ...
        if iter_dac_convrg_max_min_1(1,j) > n_all && n_all <= 5 % flooding
            ...
            s2_inv_dist_1_hop(j) = sum(l_dist_1(:,1)) + (1-sum(beta_dist_1(:,1)))*s2_comm_inv(j);
            s2_dist_1_hop(j) = (1/s2_inv_dist_1_hop(j));
            mu_dist_1_hop(j) = s2_dist_1_hop(j)*(sum(z_dist_1(:,1))...
                +(1-sum(beta_dist_1(:,1)))*s2_comm_inv(j)*mu_comm(j))...
                *models{1}.Y_std + models{1}.Y_mean;
            s2_dist_1_hop(j) = (1/s2_inv_dist_1_hop(j))*(models{1}.Y_std)^2;
        else % DAC
            s2_inv_dist_1_hop(j) = n*l_dist_1(1,iter_dac_convrg_max_min_1(1,j))...
                + (1-n*beta_dist_1(1,iter_dac_convrg_max_min_1(1,j)))*s2_comm_inv(j);
            s2_dist_1_hop(j) = (1/s2_inv_dist_1_hop(j));
            mu_dist_1_hop(j) = s2_dist_1_hop(j)*(n*z_dist_1(1,iter_dac_convrg_max_min_1(1,j))...
                +(1-n*beta_dist_1(1,iter_dac_convrg_max_min_1(1,j)))*s2_comm_inv(j)*mu_comm(j))...
                *models{1}.Y_std + models{1}.Y_mean;
            s2_dist_1_hop(j) = (1/s2_inv_dist_1_hop(j))*(models{1}.Y_std)^2;
        end
        ...
        if iter_dac_convrg_max_min_2(1,j) > n_all && n_all <= 5 % flooding
            ...
            s2_inv_dist_2_hop(j) = sum(l_dist_2(:,1)) + (1-sum(beta_dist_2(:,1)))*s2_comm_inv(j);
            s2_dist_2_hop(j) = (1/s2_inv_dist_2_hop(j));
            mu_dist_2_hop(j) = s2_dist_2_hop(j)*(sum(z_dist_2(:,1))...
                +(1-sum(beta_dist_2(:,1)))*s2_comm_inv(j)*mu_comm(j))...
                *models{1}.Y_std + models{1}.Y_mean;
            s2_dist_2_hop(j) = (1/s2_inv_dist_2_hop(j))*(models{1}.Y_std)^2;
        else % DAC
            s2_inv_dist_2_hop(j) = n*l_dist_2(1,iter_dac_convrg_max_min_2(1,j))...
                + (1-n*beta_dist_2(1,iter_dac_convrg_max_min_2(1,j)))*s2_comm_inv(j);
            s2_dist_2_hop(j) = (1/s2_inv_dist_2_hop(j));
            mu_dist_2_hop(j) = s2_dist_2_hop(j)*(n*z_dist_2(1,iter_dac_convrg_max_min_2(1,j))...
                +(1-n*beta_dist_2(1,iter_dac_convrg_max_min_2(1,j)))*s2_comm_inv(j)*mu_comm(j))...
                *models{1}.Y_std + models{1}.Y_mean;
            s2_dist_2_hop(j) = (1/s2_inv_dist_2_hop(j))*(models{1}.Y_std)^2;
        end
