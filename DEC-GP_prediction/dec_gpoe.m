...
for j=1:nt
    ...
    if   nearest_neighbors(j)  > 1
        ...
        if iter_dac_convrg_max_min(1,j) > n_all && n_all <= 5 % flooding
            ...
            s2_inv_dist_comp(j) = (1/n)*sum(l_dist_comp(:,1));
            s2_dist_comp(j) = (1/s2_inv_dist_comp(j));
            mu_dist_comp(j) = (s2_dist_comp(j)*(1/n)*sum(z_dist_comp(:,1))*models{1}.Y_std) + models{1}.Y_mean;
            s2_dist_comp(j) = s2_dist_comp(j)*(models{1}.Y_std)^2; % Normalize after multiplying to mean value
              
        else % DAC
            s2_inv_dist_comp(j) = (1/n)*n*l_dist_comp(1,iter_dac_convrg_max_min(1,j));
            s2_dist_comp(j) = (1/s2_inv_dist_comp(j));
            mu_dist_comp(j) = (s2_dist_comp(j)*(1/n)*n*z_dist_comp(1,iter_dac_convrg_max_min(1,j))*models{1}.Y_std) + models{1}.Y_mean;
        ...
        if iter_dac_convrg_max_min_1(1,j) > n_all && n_all <= 5 % flooding
            ...
            s2_inv_dist_1_hop(j) = (1/n)*sum(l_dist_1(:,1));
            s2_dist_1_hop(j) = (1/s2_inv_dist_1_hop(j));
            mu_dist_1_hop(j) = s2_dist_1_hop(j)*(1/n)*sum(z_dist_1(:,1))*models{1}.Y_std + models{1}.Y_mean;
            s2_dist_1_hop(j) = (1/s2_inv_dist_1_hop(j))*(models{1}.Y_std)^2;
        else % DAC
            s2_inv_dist_1_hop(j) = (1/n)*n*l_dist_1(1,iter_dac_convrg_max_min_1(1,j));
            s2_dist_1_hop(j) = (1/s2_inv_dist_1_hop(j));
            mu_dist_1_hop(j) = s2_dist_1_hop(j)*(1/n)*n*z_dist_1(1,iter_dac_convrg_max_min_1(1,j))*models{1}.Y_std + models{1}.Y_mean;
            s2_dist_1_hop(j) = (1/s2_inv_dist_1_hop(j))*(models{1}.Y_std)^2;
        end
        
        if iter_dac_convrg_max_min_2(1,j) > n_all && n_all <= 5 % flooding
            iter_dac_convrg_max_min_2(1,j) =  n_all;
            s2_inv_dist_2_hop(j) = (1/n)*sum(l_dist_2(:,1));
            s2_dist_2_hop(j) = (1/s2_inv_dist_2_hop(j));
            mu_dist_2_hop(j) = s2_dist_2_hop(j)*(1/n)*sum(z_dist_2(:,1))*models{1}.Y_std + models{1}.Y_mean;
            s2_dist_2_hop(j) = (1/s2_inv_dist_2_hop(j))*(models{1}.Y_std)^2;
        else % DAC
            s2_inv_dist_2_hop(j) = (1/n)*n*l_dist_2(1,iter_dac_convrg_max_min_2(1,j));
            s2_dist_2_hop(j) = (1/s2_inv_dist_2_hop(j));
            mu_dist_2_hop(j) = s2_dist_2_hop(j)*(1/n)*n*z_dist_2(1,iter_dac_convrg_max_min_2(1,j))*models{1}.Y_std + models{1}.Y_mean;
            s2_dist_2_hop(j) = (1/s2_inv_dist_2_hop(j))*(models{1}.Y_std)^2;
        end
