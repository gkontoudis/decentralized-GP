function [mu_dist_comp,s2_dist_comp,mu_dist_1_hop,...
    ...
    iter_dac_convrg_max_min_2,nearest_neighbors,omega_all]...
...
for j=1:nt
    ...
    if   nearest_neighbors(j)  > 1
        ...
        if iter_dac_convrg_max_min(1,j) > n_all && n_all <= 5 % JOR+flooding
            ...
            iter_dac_convrg_max_min(1,j) = iter_dac_convrg_max_min(1,j) * (n/n_all); % account for less agents in the aggregation from CBNN
        ...
        if iter_dac_convrg_max_min_1(1,j) > n_all && n_all <= 5
            ...
            iter_dac_convrg_max_min_1(1,j) = iter_dac_convrg_max_min_1(1,j) * (n/n_all); % account for less agents in the aggregation from CBNN
        ...
        if iter_dac_convrg_max_min_2(1,j) > n_all && n_all <= 5
            ...
            iter_dac_convrg_max_min_2(1,j) = iter_dac_convrg_max_min_2(1,j) * (n/n_all); % account for less agents in the aggregation from CBNN
    ...
    iter_jor_convrg_max_min(1,j) = iter_jor_convrg_max_min(1,j) * (n/n_all); % account for less agents in the aggregation from CBNN
    non_zero.index = [];
    L_1_nt_reduced = [];
    L_1_cbnn = [];
    epsilon_1 = [];
